...
const DataMigration: React.FC = () => {
  ...
  const handleForceSync = async () => {
    ...
    // デバッグ情報を表示
    console.log('強制同期開始', { userId: currentUser.id, isConnected });
    try {
      if (syncService) {
        // ローカルデータをSupabaseに同期
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="text-2xl font-jp-bold text-blue-600">{isAdminMode ? totalLocalDataCount : localDataCount}</span>
            </div>
          </div>
          <div className="bg-green-50 rounded-lg p-4 border border-green-200">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="text-2xl font-jp-bold text-green-600">{isAdminMode ? totalSupabaseDataCount : supabaseDataCount}</span>
            </div>
          </div>
        </div>
