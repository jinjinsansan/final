...
const AdminPanel: React.FC = () => {
  ...
  const [isVisibleToUser, setIsVisibleToUser] = useState<boolean>(false);
  ...
  const fetchEntriesFromSupabase = async () => {
    ...
    try {
      ...
      console.log('日記データを取得しました:', formattedEntries.length, '件');
      setEntries(formattedEntries);
      setFilteredEntries(formattedEntries);
      setStatus({message: `${formattedEntries.length}件のデータを取得しました`, type: 'success'});
    } catch (error) {
  ...
  const handleSyncAdminData = async () => {
    ...
    try {
      ...
      setStatus({message: 'データの同期が完了しました', type: 'success'});
    } catch (error) {
      console.error('データ同期エラー:', error);
      setStatus({
        message: 'データの同期中にエラーが発生しました: ' + (error instanceof Error ? error.message : '不明なエラー'), 
        type: 'error'
      });
    } finally {
      setIsSyncInProgress(false);
    ...
    <div className="w-full space-y-6">
      <div className="bg-white rounded-xl shadow-lg p-6">
        <div className="flex items-center space-x-3 mb-6">
          ...
          <div className="ml-auto">
            <button
              ...
              className="flex items-center space-x-2 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 text-white px-4 py-2 rounded-lg font-jp-medium transition-colors text-sm"
            >
        ...
        <Tabs defaultValue="search" value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="grid grid-cols-1 gap-1 mb-6 overflow-x-auto">
      ...
      {/* 詳細モーダル */}
      {selectedEntry && showEntryDetails && (
