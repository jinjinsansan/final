/*
  ...
  1. 新しいテーブル
    - `backup_logs` - バックアップ操作のログを記録するテーブル
      - `id` (uuid, primary key)
      - `counselor_name` (text) - バックアップを実行したカウンセラーの名前
      - `backup_type` (text) - バックアップの種類（full, user, restore等）
      - `file_name` (text) - バックアップファイルの名前
      - `file_size` (integer) - バックアップファイルのサイズ（バイト）
      - `created_at` (timestamp) - バックアップ作成日時
...
COMMENT ON COLUMN backup_logs.backup_type IS 'バックアップの種類（full, user, restore等）';
